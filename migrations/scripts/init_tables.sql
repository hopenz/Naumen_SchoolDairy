...
CREATE TYPE user_role AS ENUM ('ROLE_TEACHER', 'ROLE_STUDENT');

CREATE CAST ( varchar AS user_role) WITH INOUT AS IMPLICIT;

CREATE TABLE users (
                       id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- Автоинкрементное поле id
--                        first_name VARCHAR(255) NOT NULL, -- Поле из встраиваемого компонента Username
--                        last_name VARCHAR(255) NOT NULL, -- Поле из встраиваемого компонента Username
                       email VARCHAR(255) NOT NULL UNIQUE, -- Email с уникальным индексом
                       password VARCHAR(255) NOT NULL, -- Поле Password
--                        phone_number VARCHAR(20) UNIQUE, -- Уникальное поле phoneNumber
                       is_verified BOOLEAN, -- Поле isVerified
                       role user_role -- Поле role с ограничением по значениям
);

CREATE TABLE IF NOT EXISTS "teacher"
(
    "teacher_id"   BIGINT PRIMARY KEY REFERENCES users(id),
    "name"         varchar,
    "surname"      varchar,
    "patronymic"   varchar,
